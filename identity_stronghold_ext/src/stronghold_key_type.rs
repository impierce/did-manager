...
pub const P256_KEY_TYPE_STR: &str = "P256";
/// The P256 key type.
pub const P256_KEY_TYPE: KeyType = KeyType::from_static_str(P256_KEY_TYPE_STR);
...
impl StrongholdKeyType {
    ...
    const fn name(&self) -> &'static str {
        match self {
            ...
            StrongholdKeyType::P256 => P256_KEY_TYPE_STR,
...
impl TryFrom<&KeyType> for StrongholdKeyType {
    ...
    fn try_from(value: &KeyType) -> Result<Self, Self::Error> {
        match value.as_str() {
            ...
            P256_KEY_TYPE_STR => Ok(StrongholdKeyType::P256),
        ...
        match jwk.kty() {
            ...
            JwkType::Ec => {
                ...
                if let Ok(bls_curve) = ec_params.try_bls_curve() {
                    return match bls_curve {
                        BlsCurve::BLS12381G2 => Ok(StrongholdKeyType::Bls12381G2),
                        curve => Err(KeyStorageError::new(KeyStorageErrorKind::UnsupportedKeyType)
                            .with_custom_message(format!("{curve} not supported"))),
                    };
                } else if let Ok(ec_curve) = ec_params.try_ec_curve() {
                    return match ec_curve {
                        identity_verification::jwk::EcCurve::P256 => Ok(StrongholdKeyType::P256),
                        curve => Err(KeyStorageError::new(KeyStorageErrorKind::UnsupportedKeyType)
                            .with_custom_message(format!("{curve} not supported"))),
                    };
                } else {
                    return Err(KeyStorageError::new(KeyStorageErrorKind::UnsupportedKeyType)
                        .with_custom_message("only P256 and BLS curves are supported for signing"));
                }
            }
